...
s_block_ptr fusion(s_block_ptr b){
  ...
  da=b->next;
    ...
    da=(s_block_ptr)extend_heap(b->next,b->size);
  }
  if(da->free==0){
...
void* mm_realloc(void* ptr, size_t size)
{
  ...
  if(b1==NULL){
    ...
    s_block_ptr tes=(s_block_ptr)get_block(ptr);
    if(tes->size<size){
      if(tes->next==NULL){
	tes=(s_block_ptr)extend_heap(tes,size)->prev;
      }else{
	tes=(s_block_ptr)fusion(tes);
      }
    }
    else if(tes->size>size){
      split_block(tes,size);
    }
    return tes;
  }
}
